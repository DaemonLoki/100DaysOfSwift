...
//  Extension
//
//  Created by Stefan Blos on 11.06.19.
...
class ScriptsTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: "ScriptCell") {
            cell.textLabel?.text = script.name
            cell.detailTextLabel?.text = script.code
            return cell
        } else {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let delegate = self.delegate {
            delegate.selectedScript(script: script)
        }
